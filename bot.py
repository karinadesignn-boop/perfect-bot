...
▶ plan — записать дело с конкретной датой/временем
Когда: есть конкретное дело + дата или время, или слова «добавь», «внеси», «запиши», «поставь», «запланируй»
date = YYYY-MM-DD, time = HH:MM (если указано), text = текст дела
Примеры:
...
«28 июня внеси в план: записаться к специалисту» → date=2026-06-28, text="записаться к специалисту"
«на 25 июля добавь: анализ крови, посетить тренера» → ДВА объекта plan date=2026-07-25
...
⚠️ Несколько дел в одном дне → отдельный объект на каждое!
Примеры:
«каждое воскресенье: анализ финансов, план на неделю, инбокс» → ТРИ объекта weekly day_of_week=6
«по понедельникам планёрка» → weekly day_of_week=0, text="планёрка"
«каждую пятницу: ревью недели» → weekly day_of_week=4

▶ show_day — показать план конкретного дня (ТОЛЬКО просмотр, без записи!)
date = YYYY-MM-DD
Примеры:
...
«что у меня на этой неделе в среду» → date=ближайшая среда
«расписание на воскресенье» → date={_this_sun}

▶ show_week — показать неделю картинкой
Примеры: «план на неделю», «что на этой неделе», «скинь неделю», «вся неделя»

▶ show_week_text — показать неделю текстом
Примеры: «план на неделю текстом», «напиши неделю», «список на неделю»

▶ show_month — показать месяц картинкой (ТОЛЬКО просмотр!)
date = YYYY-MM-01
Примеры:
«план на июль» → date=2026-07-01
«покажи август» → date=2026-08-01
«что на следующий месяц» → date=следующий месяц
«скинь июнь» → date=2026-06-01
«хочу посмотреть на сентябрь» → date=2026-09-01

▶ delete — удалить конкретное дело по ключевым словам
old_text = 1-3 слова из названия дела
Примеры: «удали встречу с врачом» → old_text="встреча врач"
«убери йогу» → old_text="йога"
«вычеркни звонок маме» → old_text="звонок мама"

▶ delete_day — удалить ВСЕ планы на конкретную дату
date = YYYY-MM-DD
Примеры:
«убери всё на 25 июля» → date=2026-07-25
«очисти пятницу» → date={_this_fri}
...
━━━ ТЕКСТ ━━━
Сохраняй ДОСЛОВНО. Не переформулируй, не добавляй слова.
