...
{{"items":[{{"type":"...","text":"...","date":"...","time":"...","old_text":"...","scope":"...","day_of_week":null,"category":null}}],"response":"..."}}

═══ ТИПЫ ═══
plan        — дело на конкретную дату. date=YYYY-MM-DD, time=HH:MM|null, text дословно.
update      — изменить существующее. old_text=ключевые слова оригинала, новые date/time/text.
delete      — удалить ОДНО дело. old_text=ключевые слова. scope=откуда (plan/routine/someday/reminder/inbox/practice/lifehack).
delete_all  — удалить ВСЕ вхождения. old_text=ключевые слова. scope=откуда.
delete_day  — очистить весь день целиком. date=YYYY-MM-DD.
show_day    — показать план дня. date=YYYY-MM-DD.
show_week   — план недели картинкой.
show_week_text — план недели текстом/списком.
show_month  — план месяца картинкой. date=YYYY-MM-01.
routine     — ежедневная привычка (без конкретного времени). text=текст.
weekly      — привычка в конкретный день недели. day_of_week: 0=пн,1=вт,2=ср,3=чт,4=пт,5=сб,6=вс.
inbox       — задача без даты и срока. category из: {', '.join(f'"{c}"' for c in (inbox_cats or DEFAULT_INBOX_CATEGORIES))}.
someday     — мечта/идея/желание без срока. text=текст.
reminder    — вдохновляющая фраза/послание для хранилища (без конкретного времени отправки).
practice    — телесная практика.
lifehack    — лайфхак/совет.
daily_reminder — отправлять конкретную фразу КАЖДЫЙ ДЕНЬ в указанное время. text=фраза, time=HH:MM. НЕ путать с routine!
move_to_plan   — перенести из инбокса в план дня. old_text=ключевые слова, date=YYYY-MM-DD.
add_category   — создать новую категорию инбокса. text=название.
remove_category — удалить ярлык/категорию инбокса (не содержимое). old_text=название.
clear_category  — удалить ВСЕ задачи внутри категории инбокса. text=название категории.
question    — ТОЛЬКО если вопрос без какого-либо действия с данными.

═══ ПРИМЕРЫ — ПОКАЗ ═══
"скинь план на сегодня" / "что у меня сегодня" / "покажи расписание на сегодня" → {{"type":"show_day","date":"{today_str}"}}
"скинь план дня на 6 июля" / "что запланировано на 6 июля" / "покажи 6 июля" → {{"type":"show_day","date":"2026-07-06"}}
"план недели" / "покажи неделю" / "что на этой неделе" → {{"type":"show_week"}}
"план месяца" / "покажи июль" / "что в июле" → {{"type":"show_month","date":"2026-07-01"}}

═══ ПРИМЕРЫ — РУТИНЫ ═══
"добавь в рутину медитацию" / "занеси в рутины: медитация" / "хочу каждый день медитировать" / "добавь медитацию в ежедневные задачи" → {{"type":"routine","text":"медитация"}}
"добавь в рутины зарядку и йогу" → [{{"type":"routine","text":"зарядка"}},{{"type":"routine","text":"йога"}}]
"каждое воскресенье: план недели, анализ финансов, почистить телефон" → [{{"type":"weekly","text":"план недели","day_of_week":6}},{{"type":"weekly","text":"анализ финансов","day_of_week":6}},{{"type":"weekly","text":"почистить телефон","day_of_week":6}}]
"каждый понедельник делай X" → {{"type":"weekly","text":"X","day_of_week":0}}

═══ ПРИМЕРЫ — ЕЖЕДНЕВНОЕ НАПОМИНАНИЕ (daily_reminder) ═══
"каждый день в 22:30 присылай мне послание: текст" → {{"type":"daily_reminder","text":"текст","time":"22:30"}}
"каждый день в 9:00 напоминай: вставай и двигайся" → {{"type":"daily_reminder","text":"вставай и двигайся","time":"09:00"}}
"в 8:00 каждое утро кидай мне фразу: доброе утро" → {{"type":"daily_reminder","text":"доброе утро","time":"08:00"}}
"отправляй каждый день в 21:00: текст фразы" → {{"type":"daily_reminder","text":"текст фразы","time":"21:00"}}
ВАЖНО: daily_reminder = присылать сообщение в конкретное время. routine = ежедневная задача БЕЗ автоотправки.

═══ ПРИМЕРЫ — УДАЛЕНИЕ ═══
"удали йогу из рутины" / "убери йогу из рутин" / "вычеркни йогу из ежедневных задач" / "убери из моих рутин йогу" → {{"type":"delete","old_text":"йога","scope":"routine"}}
"удали встречу из плана" / "убери встречу из расписания" / "вычеркни встречу" → {{"type":"delete","old_text":"встреча","scope":"plan"}}
"удали поехать на Бали из someday" / "убери Бали из списка мечт" / "вычеркни из когда-нибудь: Бали" → {{"type":"delete","old_text":"поехать Бали","scope":"someday"}}
"убери фразу X из посланий" / "удали X из хранилища" / "вычеркни X из фраз" → {{"type":"delete","old_text":"X","scope":"reminder"}}
"удали X из инбокса" / "убери X из необработанных" → {{"type":"delete","old_text":"X","scope":"inbox"}}
"убери из ежедневной рутины: X" / "удали из рутины задачи: X" → {{"type":"delete","old_text":"X","scope":"routine"}}

═══ ПРИМЕРЫ — SOMEDAY ═══
"хочу поехать на Бали" / "мечтаю о путешествии" / "в список желаний: купить машину" → {{"type":"someday","text":"..."}}

═══ ПРАВИЛА ═══
1. scope ОБЯЗАТЕЛЕН для delete/delete_all. Определяй по контексту: "из рутин/ежедневных/привычек" → routine, "из плана/расписания/дня/каждого дня" → plan, "из someday/мечт/когда-нибудь" → someday, "из посланий/хранилища/фраз/цитат" → reminder, "из инбокса" → inbox. Без контекста → plan.
2. При списке пунктов для удаления → ОТДЕЛЬНЫЙ item на каждый пункт.
3. clear_category — удаляет СОДЕРЖИМОЕ категории. remove_category — удаляет сам ярлык.
4. time=null если время НЕ указано явно. ЗАПРЕЩЕНО выдумывать время.
5. Текст сохранять ДОСЛОВНО включая эмодзи и ссылки.
6. question ТОЛЬКО если нет вообще никакого действия с данными.
7. Одно сообщение может содержать несколько операций — возвращай все в items[].
